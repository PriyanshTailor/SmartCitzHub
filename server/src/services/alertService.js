    ...
    async getActiveAlerts(userLocation = null) {
        if (!hasMongo) {
            const alerts = [
                {
                    ...
                    type: 'system',
                    severity: 'low',
                    message: 'Demo mode active: showing simulated city alerts until MongoDB is connected.',
            ...
            if (randomEnvState > 0.7) {
                alerts.push({
                    id: 'alert-demo-weather',
                    type: 'transport',
                    severity: 'medium',
                    message: `Severe weather simulation for ${userLocation || 'Metro Area'}.`,
                    area: userLocation || 'City Wide',
                    timestamp: new Date().toISOString()
                })
            }
