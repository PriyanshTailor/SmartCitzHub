    ...
    const fetchReports = async () => {
      try {
        ...
        let filtered = (data || []).map((r) => ({
          ...
          ml_predicted_category: r.ml_predicted_category,
          ml_confidence_score: r.ml_confidence_score,
          is_ml_classified: r.is_ml_classified,
            ...
            <table className="w-full">
              <thead className="bg-muted border-b border-border">
                <tr>
                  ...
                  <th className="text-left px-6 py-3 font-semibold">AI Analysis</th>
              ...
              <tbody className="divide-y divide-border">
                {reports.map((report) => (
                  <tr key={report.id} className="hover:bg-muted/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      {report.is_ml_classified ? (
                        <div className="space-y-1">
                          <div className="flex items-center gap-1">
                            <span className="text-green-600">🤖</span>
                            <span className="text-xs font-medium">
                              {report.ml_predicted_category?.replace('_', ' ').toUpperCase()}
                            </span>
                          </div>
                          {report.ml_confidence_score && (
                            <span className="text-xs text-muted-foreground">
                              {(report.ml_confidence_score * 100).toFixed(1)}% confidence
                            </span>
                          )}
                        </div>
                      ) : (
