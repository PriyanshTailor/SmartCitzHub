...
export default function WasteManagementPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [pointsRes, schedulesRes, complaintsRes] = await Promise.allSettled([
          apiFetch('/api/waste/points'),
          apiFetch('/api/waste/schedules'),
          apiFetch('/api/waste/complaints/my-complaints'),
        ])

        setWastePoints(pointsRes.status === 'fulfilled' ? pointsRes.value || [] : [])
        setSchedules(schedulesRes.status === 'fulfilled' ? schedulesRes.value || [] : [])
        setUserComplaints(complaintsRes.status === 'fulfilled' ? complaintsRes.value || [] : [])
      } catch (error) {
          ...
          setUserComplaints((current) => [
            {
              ...
              location_name: complaintForm.location_name || 'Not specified',
  ...
  const getVadodaraRouteQuery = (_name, lat, lng) => {
    return encodeURIComponent(`${Number(lat)},${Number(lng)}`)
  }

  const handleGetDirections = (name, lat, lng) => {
    if (lat && lng) {
      window.open(`https://www.google.com/maps/dir/?api=1&destination=${getVadodaraRouteQuery(name, lat, lng)}&travelmode=driving`, '_blank')
    } else {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {schedules.map((schedule) => (
              <Card key={schedule.id || schedule._id} className={`p-4 ${getDayColor(schedule.collection_day)}`}>
                <h3 className="font-semibold mb-2">{schedule.location_name || schedule.location}</h3>
                <div className="space-y-2 text-sm">
                  <p>
                    <span className="text-muted-foreground">Area: </span>
                    {schedule.district || 'Vadodara'}
                  </p>
                  <p>
                    <span className="text-muted-foreground">Type: </span>
                    {schedule.waste_type || schedule.type}
                  </p>
                  <p>
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="w-full bg-transparent hover:bg-blue-500/80 mt-4"
                  onClick={() => handleGetDirections(schedule.location_name || schedule.location, schedule.latitude, schedule.longitude)}
                >
                  Get Directions
                </Button>
              </Card>
            ...
            {wastePoints.map((point) => (
              <Card key={point.id || point._id} className="p-4 hover:border-primary transition-colors">
                ...
                <Button
                  ...
                  onClick={() => handleGetDirections(point.address || point.name, point.latitude, point.longitude)}
                >
                  Get Directions
                </Button>
              </Card>
