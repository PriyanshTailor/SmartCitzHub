...
export const mapApi = {
  ...
  /**
   * Fetches the live ML environment prediction for a location.
   * GET /api/map/environment/prediction?locationId=...
   */
  getEnvironmentalPrediction: async (locationId) => {
    const suffix = locationId ? `?locationId=${encodeURIComponent(locationId)}` : ''
    return apiFetch(`/api/map/environment/prediction${suffix}`)
  },
}
